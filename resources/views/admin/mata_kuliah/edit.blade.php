...
@section('content')
  {{ Breadcrumbs::render('matkul.edit') }}
              ...
              <div class="form-group row border-bottom pb-4">
                ...
                <div class="col-sm-10">
                  <input type="text" class="form-control" name="kode_mata_kuliah"
                    value="{{ old('kode_mata_kuliah', $mata_kuliah->kode_mata_kuliah) }}" id="kode_mata_kuliah">
                </div>
              </div>
              <div class="form-group row border-bottom pb-4">
                ...
                <div class="col-sm-10">
                  <input type="text" class="form-control" name="nama_mata_kuliah"
                    value="{{ old('nama_mata_kuliah', $mata_kuliah->nama_mata_kuliah) }}" id="nama_mata_kuliah">
                </div>
              </div>
              <div class="form-group row border-bottom pb-4">
                ...
                <div class="col-sm-10">
                  <select class="form-control" name="program_study_id" id="program_study_id">
                    @foreach ($program_studies as $program_study)
                      <option {{ $mata_kuliah->program_study->id == $program_study->id ? 'selected' : null }} value="{{ $program_study->id }}">
                        {{ $program_study->nama_prody }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
